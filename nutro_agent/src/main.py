    ...
    try:
        ...
        for day in range(1, user_params.period + 1):
            menu.append({
                "day": day,
                "meals": [
                    {"meal": "Breakfast", "dish": f"{user_params.complexity.capitalize()} Pancakes"},
                    {"meal": "Lunch", "dish": f"{user_params.complexity.capitalize()} Salad"},
                    {"meal": "Dinner", "dish": f"{user_params.complexity.capitalize()} Pasta"}
                ],
                "portions": user_params.portions
            })

        return json.dumps({"gender": user_params.gender, "weight": user_params.weight, "age": user_params.age, "activity": user_params.activity, "menu": menu}, indent=4)
